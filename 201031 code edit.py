...
def processblack(img):
    ...
    vertices = np.array(
        [[(width*3/8, height), (width*5/8, height), (width*5/8, height/2), (width*3/8, height/2)]],
        dtype=np.int32)
    ...
    print(f"  Color2: {colors[0]}, count: {count}, proportion: {proportion: .2f}%")
...
def processLog(img):
    ...
    vertices1 = np.array(
        [[(width/4, (height*3)/4), ((width*2)/4, (height*3)/4), ((width*2)/4, height/2), (width/4, height/2)]],
        dtype=np.int32)
    vertices2 = np.array(
        [[((width*3)/4, height*3/4),  ((width*2)/4, (height*3)/4), ((width*2)/4, height/2), (width*3/4, height/2)]],
        dtype=np.int32)
...
cap = cv2.VideoCapture('test_nonoverlap.mp4') # 동영상 불러오기
...
while(cap.isOpened()):
    ...
    mark = mark_img(image,0 ,0, 0) # 흰색 차선 찾기
    mark = gaussian_blur(mark, 9)
    gray_img_A = grayscale(mark)
    ret2, dst2 = cv2.threshold(gray_img_A, 0, 255, cv2.THRESH_BINARY)

    mark = np.copy(image) # roi_img 복사
    mark = mark_img(image,20 ,120, 20) # 흰색 차선 찾기
    mark = gaussian_blur(mark, 9)
